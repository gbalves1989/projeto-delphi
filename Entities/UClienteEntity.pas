...
type
  ...
  public
    ...
    function fFormatCpf(const ACpf: string): string;
...
// Responsável pela validação do CPF
...
// Responsável por remover os caracteres especiais do CPF
function TClienteEntity.fReplaceCpf(const ACpf: string): string;
begin
  Result := ACpf.Replace('.', '').Replace('-', '');
end;

// Responsável por adicionar os caracteres especias ao CPF
function TClienteEntity.fFormatCPF(const ACpf: string): string;
var
  LStrippedCpf: string;
begin
  LStrippedCpf := '';
  for var I := 1 to Length(ACpf) do
  begin
    if CharInSet(ACpf[I], ['0'..'9']) then
      LStrippedCpf := LStrippedCpf + ACpf[I];
  end;

  if Length(LStrippedCpf) = 11 then
  begin
    Result := Copy(LStrippedCpf, 1, 3) + '.' +
              Copy(LStrippedCpf, 4, 3) + '.' +
              Copy(LStrippedCpf, 7, 3) + '-' +
              Copy(LStrippedCpf, 10, 2);
  end
  else
  begin
    Result := ACpf;
...
// Responsável por validar os campos de nome, endereço e cidade
